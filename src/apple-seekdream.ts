...
export class AppleSeekDreamMDM implements IMDM {
  tokenKey: string;
  ...
  getHashedPassword() {
    const hash = crypto.createHash("md5").update(MDM_PASSWORD).digest("hex");
    return crypto.createHash("md5").update(hash.slice(7, -7)).digest("hex");
  }
  ...
  async getDevice(): Promise<MDMDevice | undefined> {
    ...
    try {
      const response = await this.sendCommand(
        `/device/info?serial=${this.query.serialNumber}`
      );
      const {
        data: { detail: device },
      } = await response.json();
      ...
      return {
        ...
        lastOnlineTime: dayjs(device.last_time).format("YYYYMMDDHHmmss"),
  ...
  async getEscrowKey(): Promise<string | undefined> {
    ...
    try {
      ...
      const response = await this.sendCommand(
        `/device/activateLock?${params.toString()}`
      );
  ...
  async enableLostMode(
    ...
    try {
      const response = await this.sendCommand(
        `/device/lock?serial=${this.query.serialNumber}`,
        {
          phone: phoneNumber,
          content: content,
          footer: "",
  ...
  async refreshLocation() {
    const response = await this.sendCommand(
      `/device/location?serial=${this.query.serialNumber}`,
      {}
    );
    const data = await response.json();
  ...
  async removePassword() {
    try {
      const response = await this.sendCommand(
        `/device/clearPasscode?serial=${this.query.serialNumber}`,
        {}
      );
      const data = await response.json();
  ...
  async setWallpaper(changeable: boolean, wallpaper?: number) {
    try {
